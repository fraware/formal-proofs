...
theorem sound_u16_sqrt {a ρ_sqrt : F}
  (h_auto : auto_spec_u16_sqrt a ρ_sqrt) : spec_u16_sqrt a ρ_sqrt := by
  ...
  constructor
  · exact lt_of_le_of_lt (sqrt_le_self na) hna_lt
  · have h_le : ret_val * ret_val ≤ na := by
      clear h_ub_lt h_ub ub
      rw [sub_eq_iff_eq_add, ← cast_mul, ← cast_add] at h_lb
      rw [PRIME.nat_coe_field_inj (lt_trans hna_lt u16Limit_lt_PRIME) _ h_lb]
      · exact le_add_self
    ...
    have h_ge : ret_val ≥ na.sqrt := by
      ...
      have h_right_lt : ub + na < PRIME := add_lt_PRIME_of_lt (lt_of_lt_of_le h_ub_lt h_rc_bound)
          (lt_trans hna_lt u16Limit_lt_u128Limit)
